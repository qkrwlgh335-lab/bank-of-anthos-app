...
flowchart LR
  ...
  BUILD --> SCAN[Trivy HIGH 보고 / CRITICAL 차단]
...
4. Trivy는 수정 가능한 HIGH/CRITICAL을 보고하고 CRITICAL이 있으면 실패시킨다.
...
- CRITICAL 취약점: 실행 실패
- HIGH 취약점: 보고만 하고 현재 PoC에서는 통과
- 한 서비스라도 선택 matrix에서 실패: GitOps 승격 job 실행 안 됨
- immutable ECR tag: 같은 SHA tag 덮어쓰기 차단

검증 실행에서는 CRITICAL 취약점은 없었지만 HIGH 결과는 있었다. 따라서 성공 표시는
“현재 정의한 CRITICAL gate를 통과했다”는 뜻이며 취약점이 전혀 없다는 뜻은 아니다.

## 운영 전에 남은 항목

다음 항목은 이번 CI 성공 범위에 포함되지 않는다.

- `main` branch protection과 필수 PR/status check 강제
- HIGH 취약점 허용 기준, 예외 만료일, SBOM 및 이미지 서명
