...
class TestUserservice(unittest.TestCase):
    ...
    def test_create_user_uses_username_redis_lock_when_configured(self):
        """signup is serialized with a per-username lock across pods"""
        redis_client = MagicMock()
        signup_lock = redis_client.lock.return_value
        signup_lock.acquire.return_value = True
        signup_lock.owned.return_value = True
        self.mocked_db.return_value.get_user.return_value = None
        self.mocked_db.return_value.generate_accountid.return_value = '123'

        with patch('userservice.userservice.redis.Redis.from_url',
                   return_value=redis_client), patch(
            'userservice.userservice.open', mock_open(read_data='foo')
        ), patch(
            'os.environ',
            {
                'VERSION': '1',
        ...
        ), patch('userservice.userservice.UserDb', return_value=self.mocked_db.return_value):
            test_app = create_app().test_client()
            response = test_app.post('/users', data=EXAMPLE_USER_REQUEST.copy())

        self.assertEqual(response.status_code, 201)
        redis_client.lock.assert_called_once_with(
            'signup:jdoe', timeout=15, blocking_timeout=3
        )
        signup_lock.acquire.assert_called_once_with()
        signup_lock.release.assert_called_once_with()
