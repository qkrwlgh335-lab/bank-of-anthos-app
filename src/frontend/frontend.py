...
def create_app():
    ...
    def whereami():
        """
        Returns the EKS runtime location where this Pod is running.
    ...
    # Runtime metadata. Query the Google metadata endpoint only on GCP; doing
    # so on EKS causes unnecessary DNS failures and startup delays.
    cluster_name = os.getenv('CLUSTER_NAME', 'unknown')
    pod_name = socket.gethostname()
    node_name = os.getenv('NODE_NAME', 'unknown')
    pod_zone = os.getenv('POD_ZONE', 'unknown')
    if os.getenv('ENV_PLATFORM', '').lower() == 'gcp':
        ...
        try:
            ...
            app.logger.warning("Unable to retrieve GCP runtime metadata")
    ...
    app.jinja_env.globals.update(node_name=node_name)
